...
// get set functions
inline std::list< std::vector<T> > get_IMF_list(int list_size){
//IMF_list.reverse();
//std::reverse(IMF_list.begin(),IMF_list.end()); 
if (list_size==1){list_size=IMF_list.size();std::cout<<list_size<<" list_size\n";}; IMF_list.resize(list_size); 
return IMF_list; }; //get the T of interest
