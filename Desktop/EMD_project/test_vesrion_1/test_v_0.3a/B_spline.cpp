...
while (i<N_sample){
if((time[i]==d_extrema_time[counter])&&(counter<dim-1)){slope=(d_extrema[counter+1]-d_extrema[counter])/(d_extrema_time[counter+1]-d_extrema_time[counter]); offset=d_extrema[counter]; counter++;}//end if inside
...
new_value=(value_initial>value_b_spline_max) ? value_initial+increase_value*value_initial:value_b_spline_max;
};
...
if (value_initial>0 && value_b_spline_max<=0){new_value=value_initial+increase_value*value_initial;};
...
new_value=(value_initial<value_b_spline_min) ? value_initial-increase_value*value_initial:value_b_spline_min;};

if (value_initial<0 && value_b_spline_min<=0){
new_value=(value_initial>value_b_spline_min) ? value_b_spline_min:value_initial+increase_value*value_initial;};

if (value_initial>0 && value_b_spline_min<=0){new_value=value_b_spline_min;};
if (value_initial<0 && value_b_spline_min>0){new_value=value_initial-increase_value*value_initial;};
...
// store vector
B_k_coeff.insert(B_k_coeff.end(),&B_array[0],&B_array[4*N_sample]); //set the array in private vector
