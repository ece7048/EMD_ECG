...
template<class T>
compute_components<T>::~compute_components(){}
...
// Initialize section
c_b_spline_max=c_b_spline.front();
c_b_spline_min=c_b_spline.back();
...
//std::cout<<maximum_array[i]<<" max "<<minimum_array[i]<<" min "<<d_mean_value[i]<<" <--- mean_value\n";
i++;
}//end while
// store mean value
c_mean_value.insert(c_mean_value.end(),&d_mean_value[0],&d_mean_value[c_b_spline_max.size()]);
}//end function
...
// store mean value
c_h_comp.insert(c_h_comp.end(),&d_h_comp[0],&d_h_comp[c_mean_value.size()]);
}//end function
