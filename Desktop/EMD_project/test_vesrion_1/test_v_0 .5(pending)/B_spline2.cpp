...
////////////////////////////////////////////////////////////////////
template<class T>
B_spline<T>::B_spline(std::vector<T> t_extrema, std::vector<T> t_extrema_time, std::vector<T> t_time) :
...
T h=(d_extrema_time[dim-1]-d_extrema_time[0])/N_sample;//h: time step between the intervals
...
T a_plus_n=2*array_a_coeff[extrema_size]+array_a_coeff[extrema_size-1];//find the a(n+1) coeff
//////debug///////////////////////////////
...
while (i<N_sample){
int position=(counter)+(counter)*3;//counter to count the B_k 4*N_sample positions
if(time[i]==extrema_time[counter]){S_extrema[i]=extrema[counter]; counter++;}//end if inside

if (time[i]<extrema_time[1] && time[i]>extrema_time[0]){S_extrema[i]=a_minus_one*B_k[position-1]+array_a_coeff[0]*B_k[position]+array_a_coeff[1]*B_k[position+1]+array_a_coeff[2]*B_k[position+2];}//end if inside}//end if

if (time[i]<extrema_time[extrema_time.size()-2] && time[i]>extrema_time[1]){
	if (time[i]<extrema_time[counter]&& time[i]>extrema_time[counter-1]){S_extrema[i]=array_a_coeff[counter-1]*B_k[position-1]+array_a_coeff[counter]*B_k[position]+array_a_coeff[counter+1]*B_k[position+1]+array_a_coeff[counter+2]*B_k[position+2];}	
}//end if

if (time[i]<extrema_time[extrema_time.size()-1] && time[i]>extrema_time[extrema_time.size()-2]){S_extrema[i]=a_coeff[counter-1]*B_k[position-1]+array_a_coeff[counter]*B_k[position]+array_a_coeff[counter+1]*B_k[position+1]+a_plus_n*B_k[position+2];}//end if
...
//store vector
interp_data.insert(interp_data.end(),&S_extrema[0],&S_extrema[N_sample]); //store the array in private vector
...
new_value=(value_initial>value_b_spline_max) ? (value_initial+(std::abs(value_initial-value_b_spline_min))):value_b_spline_max;
};
//std::cout<< " value initial: "<<value_initial<< " value max_in: "<<b_spline_max[i]<< " value min_in: "<<b_spline_min[i];
//////////////mxa check///////////////////////////////
if (value_initial<0 && value_b_spline_max<=0){
new_value=(value_initial>value_b_spline_max) ? (value_initial-(std::abs(value_initial-value_b_spline_min))):value_b_spline_max;};

if (value_initial<0 && value_b_spline_max>=0){new_value=value_b_spline_max;};
if (value_initial>0 && value_b_spline_max<=0){new_value=value_initial+(std::abs(value_initial-value_b_spline_min));};

if (value_initial==value_b_spline_max){new_value=value_b_spline_max;};
if (value_initial==0 && value_b_spline_max==0){new_value=b_spline_max[i+1]/2+0.000001; if (i==initial_signal.size()-1){new_value=b_spline_max[i-1]/2+0.000001;};};

b_spline_max[i]=new_value;
////////////////min check///////////////////////////
if (value_initial>0 && value_b_spline_min>=0){
new_value=(value_initial<value_b_spline_min) ? value_b_spline_min:(value_initial+(std::abs(value_initial-value_b_spline_max)));};

if (value_initial<0 && value_b_spline_min<=0){
new_value=(value_initial>value_b_spline_min) ? value_b_spline_min:(value_initial-(std::abs(value_initial-value_b_spline_max))); };

if (value_initial>0 && value_b_spline_min<=0){new_value=value_b_spline_min;};
if (value_initial<0 && value_b_spline_min>=0){new_value=value_initial+(std::abs(value_initial-value_b_spline_max));};

if (value_initial==value_b_spline_min){new_value=value_b_spline_min;};
if (value_initial==0 && value_b_spline_min==0){new_value=new_value=b_spline_min[i+1]/2-0.000001; if (i==initial_signal.size()-1){new_value=b_spline_min[i-1]/2-0.000001;};};

b_spline_min[i]=new_value;

//std::cout<<" value max_af: "<<b_spline_max[i]<< " value min_af: "<<b_spline_min[i] <<"\n";
}//end for
}//end function
...
////////////////////////////////////////////////////////////////////
template<class T>
void Cubic_B_comp<T>::B_determine(B_spline<T> &BS1){
//initialize values
T array_extrema_time[N_sample]; //determine T array
...
T c_h=(timestep[N_sample-1]-timestep[0])/N_sample;//h: time step between the intervals
T xo=timestep[0];
...
while (i<N_sample){
//c_h=(timestep[i+1]-timestep[i]);
//for Bk-1,Bk,Bk+1,Bk+2 define: xo+h,xo,xo-h,xo-2*h.
int offset=3;
if(timestep[i]==array_extrema_time[count]){
//xo=array_extrema_time[count]/* evaluation around the initial time of interval*/;
count++;}
else{
//B_array[count2+3+offset*count2]=(1/6)*pow((timestep[i]-xo),3);
//B_array[count2+2+offset*count2]=(2*(pow(c_h,3))/3)-(1/2)*pow(((timestep[i]-c_h)-xo),2)*(2*c_h+((timestep[i]-2*c_h)-xo));
//B_array[count2+1+offset*count2]=(2*(pow(c_h,3))/3)-(1/2)*pow(((timestep[i])-xo),2)*(2*c_h-((timestep[i])-xo));
//B_array[count2+offset*count2]=(1/6)*pow((c_h-(timestep[i]-xo)),3);

B_array[count2+3+offset*count2]=test_B_case(c_h,timestep[i]-2*c_h,xo);
B_array[count2+2+offset*count2]=test_B_case(c_h,timestep[i]-c_h,xo);
B_array[count2+1+offset*count2]=test_B_case(c_h,timestep[i],xo);
B_array[count2+offset*count2]=test_B_case(c_h,timestep[i]+c_h,xo);
...
// store vector
B_k_coeff.insert(B_k_coeff.end(),&B_array[0],&B_array[(4*N_sample-4*c_extrema_time.size())]); //set the array in private vector
...
////////////////////////////////////////////////////////////////////
template<class T>
T Cubic_B_comp<T>::test_B_case(T h, T time, T xo){
T Bk=0;

if(time>=xo+2*h && time<=xo-2*h){Bk=0;};
if(time>=xo-2*h && time<=xo-h){Bk=(1/6)*pow(2*h+(time-xo),3);};
if(time>=xo-h && time<=xo){Bk=(2*(pow(h,3))/3)-(1/2)*pow((time-xo),2)*(2*h+(time-xo));};
if(time>=xo && time<=xo+h){Bk=(2*(pow(h,3))/3)-(1/2)*pow((time-xo),2)*(2*h-(time-xo));};
if(time>=xo+h && time<=xo+2*h){Bk=(1/6)*pow((2*h-(time-xo)),3);};
...
////////////////////////////////////////////////////////////////////
template<class T>
void Coeff_solver<T>::run(B_spline<T> &BS2){
...
T c_h=BS2.get_h();//h: time step between the intervals
...
B[0]=1/pow(c_h,3)*array_extrema[0];
B[c_dim-1]=1/pow(c_h,3)*array_extrema[c_dim-1];
while (j<c_dim-1){B[j]=6/pow(c_h,3)*array_extrema[j];j++;}// end while
///////debug///////////////////////////////
//std::cout <<"\n";
...
//std::cout<<"L is :\n";
//for (int o=0;o<c_dim;++o){
//for (int p=0;p<c_dim;++p){std::cout << L[p][o]<< " ";}
//  std::cout <<"\n";}
//std::cout <<"\n";
/////////////////////////////////////////////
///////debug///////////////////////////////
...
//std::cout<<"U is :\n";
//for (int o=0;o<c_dim;++o){
//for (int p=0;p<c_dim;++p){std::cout << U[p][o]<< " ";}
//std::cout <<"\n";}
//std::cout <<"\n";
/////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////
template<class T>
void Coeff_solver<T>::store(T matrix[][N_sample], MATRIX_KIND M){
